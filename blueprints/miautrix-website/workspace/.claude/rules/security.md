...
- **MFA is mandatory, not optional.** TOTP enrolment plus recovery codes is required for the single
  administrator; `EnsureMfaConfirmed` gates every `/admin` route. Recovery codes are displayed
  exactly once at enrolment. There is no other account-recovery path — that residual risk is
  accepted on record (blueprint Risk #6).
...
- **Uploads: SVG is never allowed.** The permitted set is JPEG, PNG, WebP, PDF, DOCX, ZIP. Every upload
  ...
  upload. Files live outside the web root on the `private-media` disk and are served only through
...
- PostgreSQL binds to `127.0.0.1` on the production LXC — no other service runs there besides
  Nginx, PHP-FPM, PostgreSQL, and `supervisor`. UFW allows only 80/tcp, 443/tcp and the restricted
  SSH port.
